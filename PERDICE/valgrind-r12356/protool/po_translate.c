...
#define CHECK_SIZE(size) if (size==1 || size==8 || size==16 || size==32 ||size==64 || size==128){ \
   /*Feng:size 必须符合规定*/	\
}else{	\
...
#define DESCRIBE_PUT	describeTaintData(g_instaddr,0,"PUT")
#define DESCRIBE_GET	describeTaintData(g_instaddr,0,"GET")
#define DESCRIBE_LOAD	describeTaintData(g_instaddr,addr,"LOAD")
#define DESCRIBE_RDTMP	describeTaintData(g_instaddr,0,"RDTMP")
#define DESCRIBE_UNOP	describeTaintData(g_instaddr,0,buffer)	//UNOP: stored op name in buffer
#define DESCRIBE_BINOP	describeTaintData(g_instaddr,0,buffer)	//BINOP:stored op name in buffer
#define DESCRIBE_MUX0X	describeTaintData(g_instaddr,0,"MUX0X")
#define DESCRIBE_STORE	describeTaintData(g_instaddr,addr,"STORE")
#define DESCRIBE_EXIT	describeTaintData(g_instaddr,addr,"EXIT")
#define DESCRIBE_AMD64	describeTaintData(g_instaddr,0,"CALCULATE_CONDITION")
#define DESCRIBE_X86	describeTaintData(g_instaddr,0,"CALCULATE_CONDITION")
//#define DESCRIBE_	describeTaintData(g_instaddr,addr,"")

volatile Addr g_instaddr=0;//global variable to store the address of current instruction

/*get and print the information of the operator that associate with taint data*/
void describeTaintData(Addr instaddr,Addr taint,char *inst){
	return;
	
	/*get the information of the instruction with tainted data*/
	char funcname[MAX_FILE_SIZE]={0};
	char objname[MAX_FILE_SIZE]={0};
	char filename[MAX_FILE_SIZE]={0};
	char dirname[MAX_FILE_SIZE]={0};
	char details[MAX_FILE_SIZE]={0};
	...
	Int buf_len=MAX_FILE_SIZE;
	Int value=0;
	if(taint!=0){
		value=*(Int*)(Int)(taint);
	}
	//char inst[MAX_FILE_SIZE]="inst";
	
	VG_(describe_IP)(instaddr,details,buf_len);
	//VG_(printf)("****FF:details:%s\n",details);

	VG_(printf)("%s | %s | value:0x%x | instaddr:0x%x | taintaddr:0x%x | \n", details, inst, value, instaddr, taint);
	
	/*
	0x40083D: main (sign.c:14) | t22_6841 = LOAD I32 t19_5868 | 0x3e8 | 0xffffffff | t22_6841 <- ffefffda8_unknownobj_0
	*/
	
	// isname = VG_(get_fnname)(instaddr, funcname, MAX_FILE_SIZE);
	// if(!isname){
		// /*get function name failed*/
		// VG_(printf)("****FF:get function name failed\n");
		// return;
	// }

	// isline = VG_(get_filename_linenum)(instaddr, filename, MAX_FILE_SIZE, dirname, MAX_FILE_SIZE, &dirname_available, &linenum);
	// if(!isname){
		// /*get file name and line failed*/
		// VG_(printf)("****FF:get filename and line failed\n");
		// return;
	// }

	// VG_(printf)("****FF: Taint info: instruction-> %s, addr-> %08X, file-> %s, line-> %d\n",inst,instaddr, filename, linenum);
}


Bool findStr(char *str,int len){
	char *dest="123";
	int len_dest=3;
	...
	for(i=0;i<len;i++){
		...
		for(j=0;j<len_dest;j++){
			if(str[i+j]!=dest[j]){
				found=False;	//if not the dest string, not found		
			}
		}
...
static volatile int vul_cons_counter = 0;

//in catch : global
int *addrp;
static int flag=0;
static int danger_func_flag=0;
static int format_func_flag=0;
volatile ULong call_esp = 0;
volatile ULong lib_call_addr = 0; //the addr of call
volatile int get_esp = 0;
volatile ULong tmp_call_addr = 0;	//the addr call to
...
//add 7.10
extern FormatFuncs format_funcs;
extern FormatStrs format_strs;
extern LibFuncAddr lib_func_addr;
...
//parameters
Bool fengHelperPara=True;		//for helper parameters
...
static get_linenum(Addr addr) {
	...
	if (VG_(get_filename_linenum)(addr, filename, MAX_FILE_SIZE, dirname, MAX_FILE_SIZE, &dirname_available, &linenum) == True){
		if((VG_(strcmp)(filename,"elf-init.c")!=0) && (VG_(strcmp)(filename,"start.S")!=0)) {
			//VG_(printf)("[-]path:HWY file-> %s || line-> %d\n", filename, linenum);}
			VG_(printf)("It may cause conversion error in file: %s, line: %d\n", filename, linenum);}
	    }
}
...
static varnode* getType(Tmp tmp_to, Tmp tmp1, Tmp tmp2, UInt op) {
	...
	if(item1 || item2) {
		if(item1 && item2) {
			...
			item->type= defaultType;//default set type as unknown if and only if its dependencies without info of type
			item->addr=0;
		}
	return item;
}
...
{
	int max_depend=FZ_(clo_max_bound);//get max bound
	//VG_(printf)("feng:max depend=%d\n",max_depend);
...
static void populate_guest_args(ThreadId tid) {
   ...
   */
    ...
    guest_args[tid].args[4] = ts->arch.vex.guest_ESP;
...
static VG_REGPARM(0) void helperc_load(Addr addr, Tmp tmp, Tmp tmp_to, UInt size) {
	if(is_max_bound()){return;}
	if(fengEnterHelper){VG_(printf)("feng:entered helperc_load with addr:%llu\n",addr);}
	UInt a, b, c;
	UInt d,e,f,g;
	UInt i, j, pos;
	Dep* preg=NULL;
...
static VG_REGPARM(0) void helperc_div(Tmp tmp, UInt op,Addr addr) {
	...
	if (vul_cons_counter >= MAX_VUL_NUM) {
		return ;
		}
	if(depend_of_tmp(tmp)){
		ptmp = getDep(&deptmp,tmp);
...
char func_call[200][20] = {0};
int call_count = 0;

int xx_flag = 0;
int in_format_func_lists(const char *name,int *index)
{
	int i=0,j=0;
	for(i=0; i<format_funcs.used; i++){
		if(VG_(strcmp)(format_funcs.funcname[i].name,name) == 0)
		{
			// VG_(printf)("func: %s\n", name);
			*index = format_funcs.funcname[i].danger_param;
			return 1;
		}
		/* for test
 		// else{
			// for(j=0;j<call_count;j++){
				// if(VG_(strcmp)(func_call[j], name) == 0)
					// break;
			// }
			// if (j == call_count){
				// VG_(strcpy)(func_call[j], name);
				// call_count++;
				// VG_(printf)("call func: %s\n",name);
			// }
		// }
...
int in_danger_func_lists(const char *name,int *index)
{
	int i=0;
...
//add 7.10
static VG_REGPARM(0) void helperc_foramt_func(UInt addr,const char *func_name)
{
    ...
    Addr fps[VG_(clo_backtrace_size)];
    unsigned long *re;
	ULong ebp,esp, addr_param1,tmp,check_param;
	UInt para_len=0;
	UInt compare_len = 0;
    Bool dirname_available;
    char filename[MAX_FILE_SIZE]="None", dirname[MAX_FILE_SIZE], fnname[100];
    ...
    esp = guest_args[tid].args[4] ;    

	if (!get_esp) {			//no call found  not need to detect
		return ;
		}
    if (VG_(get_fnname_if_entry)(addr, fnname, sizeof(fnname))) {
		if ( in_format_func_lists(fnname, &para_index) ) {
			check_func = True;
	}
    }
	...
	//VG_(printf)("cheking format string function :%s\npara index %d | call esp %x | call addr %x\n",fnname, para_index, call_esp, lib_call_addr);
	//VG_(printf)("cheking format string function :%s\n",fnname);
	...
	check_param = call_esp + (para_index - 1)*4;		//get the first function paras in format_funcs
	void **ptrPara=(unsigned long )check_param;		//get the value of ebp+4 and translate it into a pointer
	para_len = VG_(strlen)(*ptrPara);
	//VG_(printf)("check_param len: %d\n", para_len);
	//VG_(printf)("para: %s \n", *ptrPara);
/*	for(i=0; i<para_len; i++)
		VG_(printf)("%02X ", *((UChar*)(*ptrPara)+i));
	VG_(printf)("\n");
*/
	for(i=0; i<para_len; i++){
		for(j=0; j<format_strs.used; j++){
			if (*((UChar*)(*ptrPara+i)) == format_strs.format_string[j][0]){
				compare_len = VG_(strlen)(format_strs.format_string[j]);
				if( VG_(strncmp)(format_strs.format_string[j], (UChar*)(*ptrPara+i), compare_len) == 0){
					VG_(get_filename_linenum)(lib_call_addr, filename, MAX_FILE_SIZE, dirname, MAX_FILE_SIZE, &dirname_available, &linenum);
					VG_(printf)("XX Danger Format String Param: %s found in file %s line %d\n", format_strs.format_string[j], filename, linenum);
					/*
					for(k=0; k<para_len; k++)
						VG_(printf)("%02X ", *((UChar*)(*ptrPara+k)));	
					VG_(printf)("\n\n");				
					*/
				}
	...
	para_len = VG_(strlen)((UChar*)check_param);
	VG_(printf)("check_param len: %d\n", para_len);

	for(i=0; i<para_len; i++){
		for(j=0; j<format_strs.used; j++){
			if (*((UChar*)check_param+i) == format_strs.format_string[j][0]){
				compare_len = VG_(strlen)(format_strs.format_string[j]);
				if( VG_(strncmp)(format_strs.format_string[j], (UChar*)(check_param+i), compare_len) == 0){
					VG_(get_filename_linenum)(lib_call_addr, filename, MAX_FILE_SIZE, dirname, MAX_FILE_SIZE, &dirname_available, &linenum);
					VG_(printf)("XX Danger Format String Param: %s in file %s line %d\n", (UChar*)(check_param+i) ,filename, linenum);					
					VG_(printf)("=================\n\n");					
				}
...
static VG_REGPARM(0) void helperc_get_esp(UInt addr)
{
	unsigned char op;
	UInt tid,linenum;
    UInt nips;
	...
	if( op == 0xE8) {
		call_esp = sps[0];
		lib_call_addr = addr;
		get_esp = 1;				// indicate we have found a call function
		//VG_(printf)("call addr %x\n",lib_call_addr);
		tmp_call_addr = *(Int*)((UChar*)(addr) + 1) + addr + 5;
		}
}
...
/*
		check_param = (Addr)(sps[0] );		//获取检测参数的地址
		void **pvoid=(unsigned long long)check_param;
		VG_(printf)("para len: %d\n", VG_(strlen)(*pvoid));
		VG_(printf)("para: %s\n", *pvoid);
		for(i=0; i<5; i++)
			//VG_(printf)("%02X ", *((UChar*)(*pvoid)+i));
			VG_(printf)("%02X ", *((UChar*)(*pvoid)+i));
			VG_(printf)("\n");
	*/
	if (lib_func_addr.func_count > 0) {
		addr_call = *(Int*)((UChar*)addr + 1);
		addr_call += addr + 5;		
		for (i=0; i<lib_func_addr.func_count; i++) {
			if (lib_func_addr.func_addr[i] == addr_call)
				check_func = True;
				para_index = lib_func_addr.para_index[i] ;
		}
	} else {
		if (!get_esp) {
			return;
		...
		if (VG_(get_fnname_if_entry)(addr, fnname, sizeof(fnname))) {
			if( in_danger_func_lists(fnname,&para_index) ) {
				check_func = True;
				lib_func_addr.func_addr[lib_func_addr.func_count] = tmp_call_addr;
				lib_func_addr.para_index[lib_func_addr.func_count] = para_index;
				lib_func_addr.func_count ++;
				VG_(printf)("save call addr: %X\n", tmp_call_addr);
			}
			//VG_(printf)("addr is %x | func entry: %s \n", addr, fnname);
		} 	
		//VG_(printf)("addr is %x | func: %s | ebp is %x | esp is %x\n", addr, func_name, fps[0], sps[0]);
	}
	
	if( !check_func ) {					//仅在第二条指令处检测
		return ;
	}

	VG_(printf)("cheking danger func para index  is %d \n", para_index);
	//VG_(printf)("addr is %x | ebp is %x | esp is %x\n", addr, fps[0], sps[0]);
	if(para_index>6)
	{
		VG_(printf)("unhandled param index ,big than 6\n");
		get_esp = 0;
	...
	check_param = call_esp + (para_index - 1)*4;	//获取检测参数的地址

	void **pvoid=(unsigned long )check_param;		//get the value of ebp+4 and translate it into a pointer
	//VG_(printf)("para: %s \n", *pvoid);
	...
	if(Symbol_ready)
	{
		if (VG_(get_filename_linenum)(lib_call_addr, filename, MAX_FILE_SIZE, dirname, MAX_FILE_SIZE, &dirname_available, &linenum) == True)
...
static VG_REGPARM(0) void helperc_update_imarkaddr(UInt addr)
{
	ImarkAddr = addr;
}
...
void taintBuf(char *data, int len){
	...
	for (i = 0; i < len; i++) {
		...
		VG_(printf)("tainting byte %d (0x%08x)\n",k + i, (ULong)(data + i));
...
static VG_REGPARM(0) void wrapper_mem(UInt addr){
	int tid;
	UInt ebp,esp;
    ...
    ebp = guest_args[tid].args[3] ;
    esp = guest_args[tid].args[4] ;    
	//VG_(printf)("esp=%08X ebp=%08X\n",esp,ebp);
	
	UInt *paraAddr[4]={NULL};
	int paraCnt=2;
	int i=0,j=0;
	int offset;
	...
	for(i=0;i<paraCnt;i++){
		offset=esp+i*4;
		paraAddr[i]=(Int *)(offset);
	}
	
	for(i=0;i<paraCnt;i++){
		VG_(printf)("para addr=%08X\n",paraAddr[i]);
	}
	...
	for(i=0;i<paraCnt;i++){
		...
		data=(Char *)pointer;
		VG_(printf)("data addr:%08X\n",data);
		for(j=0;j<20;j++){
				VG_(printf)("%02X ",(UChar)data[j]);
		}
		VG_(printf)("\n");
		
		found=findStr(data,10);
		if(found){
			len=VG_(strlen)(data);
			taintBuf(data,len);
...
static VG_REGPARM(0) void wrapper_str(UInt addr){
	int tid;
	UInt ebp,esp;
...
void wrapper_mem2(Addr addr){
	Bool found=False;

	Char* paraAddr[5]={NULL};
	Char* para=0;
	Char* data=0;
	...
	for(offset=0;offset<4*5;offset+=4){
		paraAddr[ebp+offset]=NULL;
	}
	
	
	for(i=0;i<paraNum;i++){
		para=(Char*)(*paraAddr[i]);
		data=para;
...
volatile Bool is_ftp_first=True;
					 ...
					 IRType gWordTy, IRType hWordTy )
{
	...
	/*test the function findStr*/
	/* 
	named=findStr("huawei",5);
	if(named){
		VG_(printf)("found the string\n");
		VG_(exit)(1);
	} 
	*/
	
	if(FZ_(clo_is_symbol_process)==False){
		return bb_in;
	}
	...
	for (/*use current i*/; i < bb_in->stmts_used; i++) {
		...
		if(st->tag == Ist_IMark){
			...
			named = VG_(get_fnname)(inst_addr, fnname, MAX_FILE_SIZE);
			if(named){
				//VG_(printf)("found the function %s\n",fnname);
			}
			
			if((VG_(strcmp)(fnname,"memcpy")==0 ) && (named == True))
			{
				//VG_(printf)("found the function memcpy\n");
				is_ftp=True;
				//is_ftp_first=True;
			}
		}
		...
		if(st->tag == Ist_IMark && obj_num!=0 && is_first_Imark){
			inst_addr = st->Ist.IMark.addr;
			is_first_Imark = False;
		...
		if (!is_in_obj && !is_ftp) {			//not in our interesting lib .so not instrument
			return bb_in;
		}
		...
		if(True){
			...
			case Ist_IMark:
				...
				*/
				...
				if((VG_(strcmp)(fnname,"memcpy")==0 ) && (named == True)/*  && is_ftp_first */)
				{
					is_ftp_first=False;
					VG_(printf)("found the function target\n");
					add_dirty1(wrapper_mem,mkIRExpr_HWord(current_addr));
				}
				...
				// {
					...
					// add_dirty1( helperc_get_esp,mkIRExpr_HWord(current_addr) );
					// add_dirty2( helperc_func,mkIRExpr_HWord(current_addr),mkIRExpr_HWord(FZ_(func_name)) );
					// add_dirty2( helperc_foramt_func,mkIRExpr_HWord(current_addr),mkIRExpr_HWord(FZ_(func_name)) );
				// }
			...
			case Ist_WrTmp:
				...
				case Iex_RdTmp:
					...
					to = st->Ist.WrTmp.tmp;
					add_dirty1(helperc_update_imarkaddr,mkIRExpr_HWord(current_addr));
					add_dirty2(helperc_rdtmp,
				...
				case Iex_Load:
					...
					if (addr->tag == Iex_Const) {
						...
						add_dirty4(helperc_load,
                            ...
                            mkIRExpr_HWord(to),
                            mkIRExpr_HWord(size));
			            if(addr!=INVALID_ADDR && vul_cons_counter < MAX_VUL_NUM){
			...
			case Ist_Store:
                ...
                add_dirty2(helperc_store,
                    ...
                    mkIRExpr_HWord((data->tag == Iex_RdTmp) ? data->Iex.RdTmp.tmp : INVALID_TMP));
#endif // FENG_AMD64				
				if(st->Ist.Store.addr->tag == Iex_RdTmp && vul_cons_counter < MAX_VUL_NUM)
				{
					add_dirty2(helperc_pointer_arry,
