...
def compute_path_constraint(input_file):
    ...
    if not DEBUG_LAST:
        if(FZ_DebugFlag):
            print 'feng:entered call run_valgrind'
        #feng:entered problem here when run in x64
    ...
    pc = []
    if(FZ_DebugFlag):
        print 'feng:entered output_filename:%s' %output_filename
    fp = open(output_filename, 'r')
    for line in fp:
        m = re.match('\[\+\] 0x[0-9]+.* depending of input: if \((.*)\) => (\d)', line)
        if m:
            if(FZ_DebugFlag):
                print 'feng:entered m:%s' %m
...
def expand_execution(input, callbacks):
    if(FZ_DebugFlag):
        print 'feng:entered enter expand_execution'
    '''
    ...
    if not callback_start_expander:
        ...
        if(FZ_DebugFlag):
            print 'feng:entered call callback_start_expander'
    ...
    if input.bound in cva_paths:
        ...
        start = time.time()
        if(FZ_DebugFlag):
            print 'feng:entered call compute_path_constraint'
    ...
    if callback_expanded:
        if(FZ_DebugFlag):
            print 'feng:entered call callback_expanded'
        callback_expanded()
    #print '[+] here'
    ...
    if not callback_start_constraint_analyser:
        ...
        if(FZ_DebugFlag):
            print 'feng:entered call callback_start_constraint_analyser'
        callback_start_constraint_analyser(len(pc))
        
    if(FZ_DebugFlag):
        print 'feng:entered call contraint_subsumption'
    j = 1
    for (c, taken) in pc:
        #print c,'   ',taken 
        if not taken:
    ...
    if callback_start_constraint_solver:
        print 'callback_start_constraint_solver exists\n'
    ...
    for j in range(input.bound, len(constraints)):
        ...
        #    print '%s' % solution
            ...
            if stp.query:
                #print 'stp.query is True\nlen(stp.query) is ', len(stp.query), '\n'
                stp.negate(len(stp.query) - 1)
                '''
                if DEBUG_LAST:
        ...
        if solution:
            ...
            if not callback_constraint_solved:
                printable_bytes = bytes[:10]
                #print '    * new_input (%d%s): ' % (ninput, PARAM['EXTENSION'])
                #print '    ', list(new_input.bytes)
                #print '    time_taken : %s' % (elapsed)
            ...
            #f.close()
                ...
                if(FZ_DebugFlag):
                    print 'feng:entered call callback_constraint_solved'
...
def search(target, worklist, callbacks):    
    if(FZ_DebugFlag):
        print 'feng:entered enter search'
    global ninput
    global paths
    global elapsed
    global querytime
    ...
    while worklist:
        ...
        if(FZ_DebugFlag):
            print 'feng:entered call expand_execution'
...
if __name__ == '__main__':
    ...
    if(FZ_DebugFlag):
        print 'feng:entered  start search'
