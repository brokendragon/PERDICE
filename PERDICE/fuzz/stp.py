...
#from amd64g_calculate_condition import amd64g_calculate_condition
...
class STP_function:
    ...
    def pp(self):
        ...
        #try:
        if self.op in [ 'NOT', '~' ]:
            temp0=self.args[0].pp()

            return '%s(%s)' % (self.op, temp0)
        elif self.op in [ '@' ]:
            temp0=self.args[0].pp()
            temp1=self.args[1].pp()

            return '(%s%s%s)' % (temp0, self.op, temp1)
        elif self.op in [ '@', '&', '=', '|' ]:
            temp0=self.args[0].pp()
            temp1=self.args[1].pp()
            ...
            return '%s(%s,%s)' % (self.op, temp0, temp1)
        elif self.op in [ 'BVSUB', 'BVPLUS', 'BVMULT', 'BVMOD', 'SBVMOD', 'BVDIV', 'SBVDIV' ]:
            return '%s(%d,%s)' % (self.op, self.size, ','.join([ a.pp() for a in self.args ]))
        elif self.op in [ 'IF' ]:
            temp0=self.args[0].pp()
            temp1=self.args[1].pp()
            temp2=self.args[2].pp()

            return '(IF (%s) THEN (%s) ELSE (%s) ENDIF)' % (temp0, temp1, temp2)
        elif self.op in [ '<<', '>>' ]:
            temp0=self.args[0].pp()

            return '(%s %s %d)' % (temp0, self.op, self.args[1].value)
        elif self.op in [ 'BVSX' ]:
            temp0=self.args[0].pp()

            return '%s(%s,%d)' % (self.op, temp0, self.size)
        elif re.match('\[\d+:\d+\]', self.op):
            temp0=self.args[0].pp()

            return '((%s)%s)' % (temp0, self.op)
        else:
...
class STP:
    ...
    def from_expr(self, e):
        ...
        if isinstance(e, Iex_Unop):
            ...
            #Feng: add for 128HIto64 and so on
            m = re.match('(\d+)HIto(\d+)', e.op.op)
            if m:
                i = int(m.group(1))
                j = int(m.group(2))
                assert(j < i)
                args = [ self.from_expr(e.arg) ]
                return STP_function('[%d:%d]' % (i - 1, j), j, args)

            #Luo: add for Left64 and so on, x|-x
            m = re.match('Left(\d+)', e.op.op)
            if m:
                size = m.group(1)
                args[0] = [ self.from_expr(e.arg) ]
                args[1] = STP_function('BVSUB', size, [0,args[0]])
            ...
            #Feng : add clz64: counting leading zeros for 64
            if e.op.op == 'Clz64':
                arg = self.from_expr(e.arg)
                args = []
                masks = [0xAAAAAAAAAAAAAAAA,0xCCCCCCCCCCCCCCCC,0xF0F0F0F0F0F0F0F0,0xFF00FF00FF00FF00,0xFFFF0000FFFF0000,0xFFFFFFFF00000000]

                for i, mask in enumerate(masks):
                    a = STP_function('&', 64, [ arg, STP_const(mask, 64) ])
                    b = STP_function('&', 64, [ arg, STP_const((~mask) % 0x10000000000000000, 64) ])
                    bvle = STP_function('BVLE', 1, [ a, b ])
                    x = STP_const(1 << i, 64)
                    y = STP_const(0, 64)
                    cond = STP_function('IF', 64, [ bvle, x, y ])
                    args.append(cond)
                nz = STP_function('BVPLUS', 64, args)
                z = STP_const(0, 64)
                cond = STP_function('=', 64, [ arg, STP_const(0, 64) ])
                return STP_function('IF', 64, [ cond, z, nz ])

            #Feng: add ctz64: counting trailing zeros for 64 bit?????????
            if e.op.op == 'Ctz64':
                arg = self.from_expr(e.arg)
                args = []
                masks = [0xAAAAAAAAAAAAAAAA,0xCCCCCCCCCCCCCCCC,0xF0F0F0F0F0F0F0F0,0xFF00FF00FF00FF00,0xFFFF0000FFFF0000,0xFFFFFFFF00000000]

                NAndNotN=STP_function('&', 64, [ arg, STP_function('-',64,[STP_const(0, 64),arg])])

                for i, mask in enumerate(masks):
                    #a = STP_function('&', 64, [ arg, STP_const(mask, 64) ])
                    b = STP_function('&', 64, [ arg, STP_const((~mask) % 0x10000000000000000, 64) ])
                    ne = STP_function('=', 1, [ b, STP_const(0, 64) ])
                    y = STP_const(0, 64)
                    x = STP_const(1 << i, 64)
                    cond = STP_function('IF', 64, [ ne, x, y ])
                    args.append(cond)
                nz = STP_function('BVPLUS', 64, args)
                z = STP_const(0, 64)
                cond = STP_function('=', 64, [ arg, STP_const(0, 64) ])
        ...
        #二元符号操作
            ...
            #Jin: add for DivModS and so on
            m = re.match('DivModS(\d+)to(\d+)', e.op.op)
            if m:
                HIsize = int(m.group(1))
                fragsize = int(m.group(2))
                args = [ self.from_expr(e.arg1), self.from_expr(e.arg2) ]
                '''
                args[0] = dirty_patch_size(args[0], HIsize)
                args[1] = dirty_patch_size(args[1], HIsize)
                lo = STP_function('BVDIV', HIsize, args)
                '''
                args[0] = dirty_patch_size(args[0], HIsize)
                args[1] = dirty_patch_size(args[1], HIsize)
                low = STP_function('SBVDIV', HIsize, args)
                high = STP_function('SBVMOD', HIsize, args)

                newl = STP_function('[%d:%d]' %(fragsize - 1,0), fragsize, (low))
                newh = STP_function('[%d:%d]' %(fragsize - 1,0), fragsize, (high))
                #return STP_function('BVSX',HIsize,[hi,lo])
                return STP_function('BVSX',HIsize,[newh,newl])

            #Jin: add for DivModU128to64 and so on
            m = re.match('DivModU(\d+)to(\d+)', e.op.op)
            if m:
                HIsize = int(m.group(1))
                fragsize = int(m.group(2))
                args = [self.from_expr(e.arg1) , self.from_expr(e.arg2)]
                ...
                low = STP_function('BVDIV', HIsize, args)
                high = STP_function('BVMOD', HIsize, args)

                newl = STP_function('[%d:%d]' %(fragsize - 1,0), fragsize, (low))
                newh = STP_function('[%d:%d]' %(fragsize - 1,0), fragsize, (high))
                #return STP_function('BVSX',HIsize,[hi,lo])
                return STP_function('BVSX',HIsize,[high,low])

            #Feng: add for Iop_64HLto128 and so on
            m = re.match('(\d+)HLto(\d+)', e.op.op)
            if m:
                sizeLow = int(m.group(1))
                sizeHigh = int(m.group(2))
                assert(sizeLow<sizeHigh)
                args = [ self.from_expr(e.arg1), self.from_expr(e.arg2) ]
                args[0] = dirty_patch_size(args[0], sizeLow)
                args[1] = dirty_patch_size(args[1], sizeLow)
                return STP_function('BVSX', sizeHigh, args)

            #Feng: only PPC need this, delete
            '''
            #Feng: add for Iop_CmpORD32/64U/S and so on
            m = re.match('CmpORD(\d+)([SU]?)', e.op.op)
            if m:
                args = [ self.from_expr(e.arg) ]
                i = int(m.group(1))
                j = int(m.group(2))
            ...
            # size (eg. LDle:I8(input(0)) AND LDle:I64(input(0)))
    ...
    def pp(self):
        ...
        #feng：加入所有的条件，合成query，修改一条指令为多条
        #s += 'QUERY(NOT(%s));' % ' AND '.join([ q.pp() for q in query ])
        s += 'QUERY(NOT('
        for q in query:
            '''
            if(q==None):
                print "nani!it is None!"
                exit(-1)
            '''
            temp=q.pp();
            #print temp
            ...
            #feng：不允许query中存在None，有则跳过
            if(temp.find("None")!=-1):
                #print "stp.pp():q.pp() is None"
                continue;
            else:
                s+='%s' % temp+' AND '
    ...
    def execute(self):
        ...
        #print self.pp()
        fp = open('./output/stp_input.txt', 'w')
        fp.write(self.pp())
        fp.close()
        #print 'stp prog ',STP_PROG
    ...
    def interpret(self):
        ...
        '''
        #print "feng:result=%s" % self.result
        lines = self.result.split('\n')

        #feng: ask if the solution exist - valid means no solution
        if lines[0] == 'Valid.':
            return None
        else:
            solution = {}
            for l in lines[1:-1]:
               ...
               # print "feng:re.sub=l=%s" % l
                ...
                if m:
                    ...
                    continue#Feng CT: we pass the un-matched string
