...
def parse_expr(e):
    ...
    n = Node('ROOT', [ e ])
    original_expr = e # just for debugging purpose
    thread.start_new_thread(timer,(10,))

    if(e==None):
        #print "enter in valgrind parse_expr expr=%s" % e
        timer_flag = False
        return None

    while True:
        if time_out:
            print "parse time out ~~~~~~\n"
            timer_flag = False
            return None
        if len(n.expr) != len(n.arg):
            s = n.arg[len(n.expr)]

            #print "in valgrind expr=%s" %(e)
            if not s.find('(') > -1:
            ...
            if len(n.expr) == 1:
                ...
                if(n.op == 'x86g_calculate_condition'):
                    ...
                    #print "in valgrind expr=%s" % expr
                    if(expr==None):
                        #print "in valgrind expr==None"
                        timer_flag = False
                        return None
                elif(n.op == 'amd64g_calculate_condition'):
                    assert(isinstance(n.expr[0], Iex_Const))
                    assert(isinstance(n.expr[1], Iex_Const))
                    cond = n.expr[0]
                    cc_op = n.expr[1]
                    # XXX - arg, this is recursive
                    # fix amd64g_calculate_condition.py
                    #feng:changed
                    #if(len(n.expr)==4):
                    #print "calculate condition with args 2 3"
                    expr = parse_expr(amd64g_calculate_condition(cond, cc_op, n.expr[2].pp(), n.expr[3].pp()))
                    #print "in valgrind expr=%s" % expr
                    if(expr==None):
                        #print "in valgrind expr==None"
                        timer_flag = False
                        ...
                        print "calculate condition with args 2 3 4"
                        expr = parse_expr(amd64g_calculate_condition(cond, cc_op, n.expr[2].pp(), n.expr[3].pp(),n.expr[4].pp()))
                        '''
                else:
                    assert(False)
            else:
...
def run_valgrind_check(progname, progarg, input_file,max_constraint=-1):
    tmp_dir = './output'
    if os.path.isdir(tmp_dir):
        tmp_filename = './output/valgrind_check_%d.txt' % os.getpid()
    else:
        os.mkdir(tmp_dir)
        tmp_filename = './output/valgrind_check_%d.txt' % os.getpid()
    ...
    file_para = "--filename="+input_file
    arg_valgrind = [ FUZZGRIND, '--tool=none', file_para ]
    stdin = open(input_file, 'r')

    if(FZ_PRINT_ARG) :print "arg:\n%s %s )" % (' '.join(arg_valgrind),' '.join(arg_prog))
...
def run_valgrind(progname, progarg, input_file, sys_module,object_list,is_symbolize,taint_stdin=False, max_constraint=-1):
    #print 'feng:entered enter run_valgrind'
    '''
    ...
    tmp_dir = './output'
    if os.path.isdir(tmp_dir):
        tmp_filename = './output/valgrind_output_%d.txt' % os.getpid()
    else:
        os.mkdir(tmp_dir)
        tmp_filename = './output/valgrind_output_%d.txt' % os.getpid()

    module = '--module-filter=None'
    obj_list = '--obj-list=None'

    if sys_module != '':
        module = '--module-filter=' + sys_module
    if object_list != '':
        obj_list = '--obj-list=' + object_list

    #print "parameter:",progname,progarg,input_file
    if not progarg:
        arg_prog = [ progname ]
        if not taint_stdin:
            arg_prog.append(input_file)
    ...
    if not taint_stdin:
        arg_valgrind = [ FUZZGRIND, '--tool=fuzzgrind', '--taint-file=yes', module,obj_list, '--file-filter=' + input_file ]
        stdin = None
    else:
        arg_valgrind = [ FUZZGRIND, '--tool=fuzzgrind', '--taint-stdin=yes', module,obj_list ]
        stdin = open(input_file, 'r')

    if is_symbolize:
        arg_valgrind.append('--symbolize=yes')
    if(FZ_PRINT_ARG) :print "arg:\n%s %s )" % (' '.join(arg_valgrind),' '.join(arg_prog))
    #print 'feng:entered tmp_filename:%s' %tmp_filename
    fp = open(tmp_filename, 'w')
    p = subprocess.Popen(arg_valgrind + arg_prog, stdin=stdin, stdout=subprocess.PIPE, stderr=fp.fileno())
