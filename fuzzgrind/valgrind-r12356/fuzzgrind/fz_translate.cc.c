...
extern Func_info function_info;
...
static void populate_guest_args(ThreadId tid) {
    ThreadState *ts =  VG_(get_ThreadState) (tid);
    guest_args[tid].args[1] = ts->arch.vex.guest_RDI;				//64位函数调用第一个参数
    guest_args[tid].args[2] = ts->arch.vex.guest_RSI;				//参数2
...
static VG_REGPARM(0) void helperc_div(Tmp tmp2, UInt op,Addr addr) {
    ...
    switch (op)
    {
     ...
     switch(op)
     {
        ...
        case Iop_DivModU128to64:
             ...
             if(VG_(strlen)(getDep(&deptmp,tmp2)->cons)>=1 && linenum != 0)
             {
                 VG_(printf)("[+] 0x%08x depending of input: if (CmpEQ%d(%s,0x0:I%d)) => 3\n",(UInt)addr,size,size,getDep(&deptmp,tmp2)->cons,size);
        ...
        case Iop_DivModS128to64:
            ...
            if(VG_(strlen)(getDep(&deptmp,tmp2)->cons)>=1 && linenum != 0)
            {
                VG_(printf)("[+] 0x%08x depending of input: if (CmpEQ%d(%s,0x0:I%d)) => 3\n",(UInt)addr,size,getDep(&deptmp,tmp2)->cons,size);
                VG_(printf)("detect fault in %s, fault type is 0, line num is %d\n",filename,linenum);
            }
            break;
        default:
...
// Add by Luo
static VG_REGPARM(0) void helperc_pointer_store(Tmp tmp,Addr current_addr)
{
    UInt tid;
    UInt linenum = 0;
    Bool dirname_available;
	...
	//if(linenum !=0 )VG_(printf)("entered helperc_pointer_store: in line %d\n",linenum);

	if(tmp != INVALID_TMP && VG_(strlen)(getDep(&deptmp,tmp)->cons) >1  && linenum != 0)
    {
#if defined(VGA_x86)

         VG_(printf)("[+] 0x%08x depending of input: if (32to1(1Uto32(CmpEQ32(LDle:I32(STle(%s)),0x0:I32)))) => 3\n",current_addr,getDep(&deptmp,tmp)->cons);
        VG_(printf)("detect fault in %s, fault type is 1, line num is %d\n",filename,linenum);

         VG_(printf)("[+] 0x%08x depending of input: if (32to1(1Uto32(CmpEQ32(LDle:I32(STle(%s)),0x%08x:I32)))) => 2\n",current_addr,getDep(&deptmp,tmp)->cons,guest_args[tid].args[3]);
        VG_(printf)("detect fault in %s, fault type is 2, line num is %d\n",filename,linenum);
#elif defined(VGA_amd64)

         VG_(printf)("[+] 0x%08x depending of input: if (64to1(1Uto64(CmpEQ64(LDle:I64(STle(%s)),0x0:I64)))) => 3\n",current_addr,getDep(&deptmp,tmp)->cons);
        VG_(printf)("detect fault in %s, fault type is 1, line num is %d\n",filename,linenum);

         VG_(printf)("[+] 0x%08x depending of input: if (64to1(1Uto64(CmpEQ64(LDle:I64(STle(%s)),0x%08x:I64)))) => 2\n",current_addr,getDep(&deptmp,tmp)->cons,guest_args[tid].args[3]);
        VG_(printf)("detect fault in %s, fault type is 2, line num is %d\n",filename,linenum);
...
// add by Luo index out of range check
static VG_REGPARM(0) void helperc_pointer_arry(Addr addr, Tmp tmp, UInt size,Addr current_addr) {
    ...
    if(VG_(strlen)(getDep(&deptmp,tmp)->cons) >1  && linenum != 0)
    {
        VG_(printf)("deptmp cons %d :%s\n",tmp,getDep(&deptmp,tmp)->cons);
        VG_(printf)("[+] 0x%08x depending of input: if (32to1(1Uto32(CmpEQ32(LDle:I32(STle(%s)),0x0:I32)))) => 3\n",current_addr,getDep(&deptmp,tmp)->cons);

        VG_(printf)("detect fault in %s, fault type is 1, line num is %d\n",filename,linenum);

        VG_(printf)("[+] 0x%08x depending of input: if (32to1(1Uto32(CmpLE32U(LDle:I32(STle(%s)),0x%08x:I32)))) => 2\n",current_addr,getDep(&deptmp,tmp)->cons,fps[0]);
        VG_(printf)("detect fault in %s, fault type is 2, line num is %d\n",filename,linenum);
    }
#elif defined(VGA_amd64)
    if(VG_(strlen)(getDep(&deptmp,tmp)->cons) >1  && linenum != 0)
    {
        VG_(printf)("deptmp cons %d :%s\n",tmp,getDep(&deptmp,tmp)->cons);
        VG_(printf)("[+] 0x%08x depending of input: if (64to1(1Uto64(CmpEQ64(LDle:I%d(STle(%s)),0x0:I64)))) => 3\n",current_addr,size,getDep(&deptmp,tmp)->cons);

        VG_(printf)("detect fault in %s, fault type is 1, line num is %d\n",filename,linenum);

        VG_(printf)("[+] 0x%08x depending of input: if (64to1(1Uto64(CmpLE64U(LDle:I%d(STle(%s)),0x%08x:I64)))) => 2\n",current_addr,size,getDep(&deptmp,tmp)->cons,fps[0]);
        VG_(printf)("detect fault in %s, fault type is 2, line num is %d\n",filename,linenum);
    }
...
static VG_REGPARM(0) void helperc_func(UInt addr,const char *func_name)
{
    ...
    if (VG_(get_fnname_if_entry)(addr, fnname, sizeof(fnname)))
	{
		if( is_in_function_list(fnname,&para_index) )
		{
			...
			re = (void*)(sps[0]);
			func_addr = *re - 5;
			//VG_(printf)("addr is %x ebp is %x sps[0] is %x flag is %d\n",addr,(unsigned long long )ebp,*re,stpcpy_flag);
	...
	else if( is_in_function_list(func_name,&para_index) )
	{
			stpcpy_flag ++;	
			nips = VG_(get_StackTrace)(VG_(get_running_tid)(), ips, VG_(clo_backtrace_size), sps, fps, 0);
			//VG_(printf)("addr is %x ebp is %x sps[0] is %x flag is %d\n",addr,(unsigned long long )ebp,sps[0],stpcpy_flag);
			check_param = (Addr)(ebp);		//获取检测参数的地址
	}
	...
	nips = VG_(get_StackTrace)(VG_(get_running_tid)(), ips, VG_(clo_backtrace_size), sps, fps, 0);
	//VG_(printf)("index is %d ebp is %x sps[0] is %x\n",para_index,(unsigned long long )ebp,sps[0]);
	VG_(printf)("cheking~~~para is %d\n",para_index);
	//para_index = function_info.func[func_index].danger_param;
	if(para_index>6)
	{
	...
	//VG_(printf)("addr is %x\nebp is %llx\ncheck param is %llx\n",addr,(unsigned long long)ebp,*pvoid);
	//for(i=0; i<16; i++)
	//	VG_(printf)("param[%d]: %x\n",i,*((unsigned char* )check_param+i));
    ...
    populate_guest_args(tid);
	ebp =  guest_args[tid].args[3];
    //addr_param1 = guest_args[tid].args[1];
    //addr_param2 = guest_args[tid].args[2];
    ...
    for (/*use current i*/; i < bb_in->stmts_used; i++) {
        ...
        if(True)
        {
                ...
                case Iex_Load:
                    ...
                    if (addr->tag == Iex_Const) {
                        ...
                        add_dirty4(helperc_load,
                            ...
                            mkIRExpr_HWord(size));
			            if(addr!=INVALID_ADDR){
						add_dirty4(helperc_pointer_arry,
                                       addr,
                                       mkIRExpr_HWord(addr->Iex.RdTmp.tmp),
                                       mkIRExpr_HWord(size),
            ...
            case Ist_Store:
				...
				{
					add_dirty2(helperc_pointer_store,
