...
char format_func_config[50] = "./config/format_func_config.ini"	;
char format_str_config[50] = "./config/format_str_config.ini"	;
...
FormatFuncs format_funcs;
FormatStrs format_strs;
LibFuncAddr lib_func_addr;
/*
static void init_FormatFuncs()
{
	VG_(strcpy)(format_funcs.funcname[0] ,"printf");
	VG_(strcpy)(format_funcs.funcname[1] , "sprintf");
	VG_(strcpy)(format_funcs.funcname[2] , "fprintf");
	VG_(strcpy)(format_funcs.funcname[3] , "snprintf");
	VG_(strcpy)(format_funcs.funcname[0] ,"vfprintf");
	format_funcs.used = 5;
...
Bool	FZ_(clo_is_symbol_process)	=	False;	//选择当前进程是否符号执行

//add 7.10
static void get_format_para_info(void)		//get the para strind to be detected in the config file
{
	SysRes func_file;
	int fd,read_size;
	char buffer[1024];
    ...
    func_file = VG_(open)(format_str_config,VKI_O_RDONLY,0);
    if(sr_isError(func_file))
	{
		VG_(printf)("Open format_func_config file failed!\n");
		return ;
	}
	...
	fd = sr_Res(func_file);
	VG_(lseek)(fd,0,SEEK_SET);
	read_size = VG_(read)(fd,buffer,1024) ;
	tmp1 = VG_(strtok)(buffer,"#");
	
	while(tmp1 != NULL){
		
		if(VG_(strcmp)("\n",tmp1) != 0 ){
			VG_(printf)("%s\n", tmp1);
			VG_(strcpy)(format_strs.format_string[format_strs.used], tmp1);
			tmp1 = VG_(strtok)(NULL, "#");
			format_strs.used++;			
		}
		else{
			tmp1 = VG_(strtok)(NULL, "#");
...
static void get_format_func_info(void)				//从配置文件中读取危险函数信息
{
	SysRes func_file ;
	int fd,read_size ;
	char buffer[2048];
	char *tmp1 = NULL, *tmp2 = NULL,*buff=NULL;
	char *out = NULL;
    func_file = VG_(open)(format_func_config,VKI_O_RDONLY,0);
    if(sr_isError(func_file))
	{
		VG_(printf)("Open format_func_config file failed!\n");
		return ;
	...
	while( tmp1 != NULL )
	{
		...
		VG_(strcpy)(format_funcs.funcname[format_funcs.used].name,tmp2);
		tmp2 = VG_(strtok)(NULL,",");
		format_funcs.funcname[format_funcs.used].danger_param = *tmp2 - '0';
		format_funcs.used ++;
		VG_(printf)("%s : %d\n",format_funcs.funcname[format_funcs.used-1].name,format_funcs.funcname[format_funcs.used-1].danger_param);
		buff = NULL;
		tmp1 = VG_(strtok_r)(buff,"#",&out);
	}
...
static void symbol_process(void){
	Int processCnt=7;
	Char*	process_args[7]={
		"",//proto2 is the process
		"",
		"",
		"",
		"",
	...
	Char client_args[1024]={0};
	int i=0;
	
	for(i=0;i<processCnt;i++){
		client_args;//get the args of current process
		
		if(VG_(strcmp)(process_args[i],client_args)==0){
			FZ_(clo_is_symbol_process)=True;
		}
	}
...
static void fz_pre_clo_init(void)
{
	...
	// init_FormatFuncs();
	get_format_para_info();
	get_format_func_info();
	lib_func_addr.func_count = 0;
