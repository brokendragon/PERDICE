...
static void cg_fini(Int exitcode)
{
  ...
  // }
   ...
   /*if((VG_(strcmp)(currFile,"elf-init.c")!=0) && (VG_(strcmp)(currFile,"start.S")!=0)) {
      VG_(sprintf)(fmt, "%%s %%,%s\n", currFile);
      VG_(umsg)(fmt, "file: ", currFile);}*/
  ...
  // VG_(sprintf)(fmt, "%%s %%,%dllu\n", l1);
   ...
   /* If cache profiling is enabled, show D access numbers and all
      miss numbers */
	//VG_(printf)("file: %s\n", currFile);
   if ((clo_cache_sim) && (lineCC->loc.line != 0) && (VG_(strcmp)(currFile,"elf-init.c")!=0) && (VG_(strcmp)(currFile,"start.S")!=0) ) {
      //VG_(sprintf)(fmt, "%%s ,%s\n", currFile);
      //VG_(umsg)(fmt, "file: ", currFile);
      VG_(printf)("file: %s\n", currFile);                  
      VG_(sprintf)(fmt, "%%s %%,%dllu\n", l1);
  
      VG_(umsg)(fmt, "line:   ", lineCC->loc.line);
   ...
   if ((clo_branch_sim) && (lineCC->loc.line != 0) && (VG_(strcmp)(currFile,"elf-init.c")!=0) && (VG_(strcmp)(currFile,"start.S")!=0) ) {
      /* Make format string, getting width right for numbers */
